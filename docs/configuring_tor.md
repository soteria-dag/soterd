...
soterd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, soterd supports a hybrid,
...
While it is easier to only run as a client, it is more beneficial to the Soter
...
Configuring soterd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the soterd command
line or in the soterd configuration file.  Typically the Tor proxy address will be
...
By default, soterd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since soterd will act as the server.
...
```text
HiddenServiceDir /var/tor/soterd
HiddenServicePort 8333 127.0.0.1:8333
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring soterd as a Tor hidden service requires
three flags:
...
```bash
$ ./soterd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
soterd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the soterd command line or in the soterd
...
```bash
$ ./soterd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
soterd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./soterd --proxy=127.0.0.1:9050 --torisolation
```
