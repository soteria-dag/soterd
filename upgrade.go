...
// oldSoterdHomeDir returns the OS specific home directory soterd used prior to
// version 0.3.3.  This has since been replaced with soterutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldSoterdHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "soterd")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".soterd")
	}
...
// location prior to soterd version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to soterd
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldSoterdHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "soterd.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "soterd_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "soterd_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to soterd
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		soterdLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old soterd.conf into new location if needed.
		...
		if ohpEmpty {
			...
			soterdLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to soterd as new versions require it.
