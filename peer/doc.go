...
caller to specify things such as the user agent name and version, the soter
network to use, which services it supports, and callbacks to invoke when soter
...
In order to do anything useful with a peer, it is necessary to react to soter
...
For convenience, a callback hook for all of the currently supported soter
...
function which allows a soterlog.Logger to be specified.  For example, logging at
...
This package supports all BIPS supported by the btcd wire package.
