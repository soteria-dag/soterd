...
/*
Package wire implements the soter wire protocol.

Soter protocol is currently similar to the bitcoin protocol. For the complete
details of the bitcoin protocol, see the official wiki entry at
https://en.bitcoin.it/wiki/Protocol_specification.  The following only serves
...
supported soter messages to and from the wire.  This package does not deal
with the specifics of message handling such as what to do when a message is
received.  This provides the caller with a high level of flexibility.

Soter Message Overview

The soter protocol consists of exchanging messages between peers.  Each
message is preceded by a header which identifies information about it such as
which soter network it is a part of, its type, how big it is, and a checksum
...
of the currently supported soter messages are provided.  For these supported
messages, all of the details of marshalling and unmarshalling to and from the
wire using soter encoding are handled so the caller doesn't have to concern
themselves with the specifics.

Message Interaction

The following provides a quick summary of how the soter messages are intended
...
and write soter messages.  The following sections provide a quick overview of
...
The soter network is a magic number which is used to identify the start of a
message and which soter network the message applies to.  This package provides
the following constants:

	wire.MainNet
	wire.TestNet  (Regression test network)
	wire.TestNet1 (Test network version 3)
	wire.SimNet   (Simulation test network)

Determining Message Type

As discussed in the soter message overview section, this package reads
and writes soter messages using a generic interface named Message.  In
...
In order to unmarshall soter messages from the wire, use the ReadMessage
function.  It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a soter peer.  Example syntax is:
...
In order to marshall soter messages to the wire, use the WriteMessage
function.  It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a soter peer.  Example syntax to request addresses
