...
// MsgNotFound defines a soter notfound message which is sent in response to
...
func (msg *MsgNotFound) SotoDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxInvPerMsg {
		...
		return messageError("MsgNotFound.SotoDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgNotFound) SotoEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	// Limit to max inventory vectors per message.
	count := len(msg.InvList)
	if count > MaxInvPerMsg {
		str := fmt.Sprintf("too many invvect in message [%v]", count)
		return messageError("MsgNotFound.SotoEncode", str)
	}
...
// NewMsgNotFound returns a new soter notfound message that conforms to the
