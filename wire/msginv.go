...
// MsgInv implements the Message interface and represents a soter inv message.
...
func (msg *MsgInv) SotoDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxInvPerMsg {
		...
		return messageError("MsgInv.SotoDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgInv) SotoEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	// Limit to max inventory vectors per message.
	count := len(msg.InvList)
	if count > MaxInvPerMsg {
		str := fmt.Sprintf("too many invvect in message [%v]", count)
		return messageError("MsgInv.SotoEncode", str)
	}
...
// NewMsgInv returns a new soter inv message that conforms to the Message
...
// NewMsgInvSizeHint returns a new soter inv message that conforms to the
